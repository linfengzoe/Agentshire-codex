...
- 任务结束后，工位释放，工作 NPC 离开办公室；镇长/观察者不会被强制离开办公室。
...
镇长代表用户或观察者，不是执行代码的 Agent。镇长可以在小镇和办公室中观察、点击 NPC 或屏幕、发起聊天。任务 NPC 收工离开后，镇长仍保留在当前观察场景，是否离开办公室由用户交互决定。
...
- 顶部工作条显示当前活跃任务数量。
- PC 屏幕可点击查看该居民本次任务、身份/职业、当前状态、最近工具调用、文件和活动摘要。
...
- 多个子 Agent 会按居民职业/人设尽量匹配到合适居民，并分配不同 PC 工位。
- 居民移动到已有 PC 工位并坐下工作。
- 头顶气泡实时显示子 Agent 的文本进度和工具阶段。
- 点击 PC 屏幕可以查看任务说明、居民身份、最近工具调用、文件和完成状态。
- 子任务完成后 NPC 离开办公室，工位被释放。
- 顶部工作条、聊天流和工位状态最终归零；镇长/观察者不被自动带离办公室。
